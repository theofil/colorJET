...
#fileName = '/store/mc/RunIISummer16MiniAODv3/QCD_HT100to200_TuneCUETP8M1_13TeV-madgraphMLM-pythia8/MINIAODSIM/PUMoriond17_94X_mcRun2_asymptotic_v3-v1/40000/F03A10A4-851E-E911-A349-0CC47A713996.root'


### read localfile
#localfile = "qcdfiles.txt"
#with open(localfile) as f: localfiles = f.readlines()
#fileNames = ['root://cms-xrd-global.cern.ch/'+filename[:-1] for filename in localfiles]
###
...
fileNames = ['/eos/cms/store/cmst3/user/gpetrucc/l1tr/105X/NewInputs104X/010319/VBFHToBB_PU200/inputs104X_VBFHToBB_PU200_job1.root']
...
dryrun       = False
maxfiles     = 5
folders   = ['ZZ', 'VBFHinv', 'Minbias','QCD', 'VBFHbb','QCDbinned']
folder    = folders[-2]
maxEvents =  0
debug = False
useAK8 = False
chargedOnly = False
jcPtMin     = 2
jcEtaMax    = 4  # TRK up to 4.0
jetPtMin    = 30
jetEtaMax   = 3  # HGCAL is up to 3
jetPtMin    = 30
...
if not dryrun:
    ...
    # start analysis
    for ii, fileName in enumerate(fileNames[:maxfiles]):
        if minimalPrint: print('opening [%d]:'%ii, fileName)
        if ii == 1 and not minimalPrint : print('opening [%d]:'%ii, fileName)  # print first
        if ii == len(fileNames)-1 and not minimalPrint: print('opening [%d]:'%ii, fileName) # print last
        events = Events(fileName)
        if minimalPrint: print('opened [%d]:'%ii, fileName)
        count = counter()
        ...
        ### ntpl vars
        outfile = 'tree_'+str(ii)+'.root' if folder =='' else 'tree_'+folder+'_'+str(ii)+'.root'
        treefile = ROOT.TFile(outfile, 'recreate')
        ...
        for iev,event in enumerate(events):
            if minimalPrint and iev%1000 == 0: print('opening [%d]:'%ii, fileName)
            count.alleve += 1
            reset()
            ...
            if edmStyle == 'AOD':
                ...
                if useAK8: event.getByLabel("ak8GenJetsNoNu", genj)
                else: event.getByLabel("ak4GenJetsNoNu", genj)         
        
            if edmStyle == 'MiniAOD':
                event.getByLabel("packedGenParticles", genp)            
        
                if useAK8: event.getByLabel("slimmedGenJetsAK8")
                else: event.getByLabel("slimmedGenJets", genj)   
            ...
            # get genJets
            genjs   = [ g for g in genj.product() if abs(g.eta())<jetEtaMax  and g.pt() > jetPtMin] 
            ...
            for j in genjs: 
                if chargedOnly: [j.daughter(d) for d in range(j.numberOfDaughters()) if (dauPtrExists(j, d) and d.charge() !=0 and d.pt() > jcPtMin and abs(d.eta()) < jcEtaMax)]
                else: jetconst = [j.daughter(d) for d in range(j.numberOfDaughters()) if dauPtrExists(j, d)]
...
def printInfo():
    myjets = [b1 , b2]
    for myjet in myjets:
        myjetjcs = [myjet.daughter(d) for d in range(myjet.numberOfDaughters()) if dauPtrExists(myjet, d)] 
        myjetjcsp4 = [[myjetc.pt(), myjetc.eta(), myjetc.phi(), myjetc.mass(), myjetc.rapidity()] for myjetc in myjetjcs]
        
        print('----------')
        print('[pt, eta, phi, M, Y] = [%f, %f, %f, %f, %f]'%(myjet.pt(), myjet.eta(), myjet.phi(), myjet.mass(), myjet.rapidity()))
        print('[pv_magnitude, pv_angle] = [%f, %f]'%(myjet.pvm, myjet.pva))
        print('[pv_x (=Y), pv_y (=Phi)] = [%f, %f]'%(myjet.pv1, myjet.pv2))
        print('')
        print('constituents [[pt, eta, phi, M, Y], [...], [...]')
        print(myjetjcsp4)
        

    print('relative pull angle (1,2) %f'%RPA(myjets[0],myjets[1]))
    print('relative pull angle (2,1) %f'%RPA(myjets[1],myjets[0]))
